...
# Paul Ambrose Additions

'''PA WINDOW - 19/02/2019

creates a graphics window

definition:
    win_obj = str, name of the window object to be passed to draw other objects
    name = str, name on the title bar of the window
    colour = str, background colour of the window
    gui_x = int, x-coord of the graphics window*
    gui_y = int, y-coord of the graphics window*

    *note, regardless of size, coords metrics are set to 0 to 1000, x and y'''


class window_pa:
    def __init__(self, win_obj, name, colour, gui_x, gui_y):
    ...
    def create_window_pa(win_obj, name, colour, gui_x, gui_y):

        win_obj = GraphWin(name, gui_x, gui_y)
        win_obj.setCoords(0.0, 0.0, 1000, 1000)
        win_obj.setBackground(colour)
        return win_obj

'''#PA BUTTON - 19/02/2019

creates a button to be added to a graphics window

definition:
    wind_obj = str, the window object (stored in a variable) of the window
               you want to draw the button into
    bot_lft_x = int, x coordinate for the first point of the rectangle which is
                the bottom left corner (range 0-1000)
    bot_lft_y = int, y coordinate for the first point of the rectangle which is
                the bottom left corner (range 0-1000)
    top_rgt_x = int, x coordinate for the second point of the rectangle which
                is the top right corner (range 0-1000)
    top_rgt_y = int, y coordinate for the second point of the rectangle which
                is the top righ corner (range 0-1000)
    lne_col = str, colour of the button outline
    fill_col = str, colour of the button background

#PA BUTTON_ARRAY - 14/03/2019

clicked_true
takes a button object and a user mouse click.  Checks if the button object has
been clicked, if it has turns this button red, and returns the button name.  If
not returns "none".

definition:
    button_obj_name = object to be tested to check if user mouse click overlaps
                      and it has therefore been selected.
    mouseclick      = getmouse user seledtion, point object

button_array_pa
creates 1 to a user defined number of buttons, returns a dictionary of the
button sequence key 1 to x, and button rectangle object which includes points

definition:
    win_obj            = name of the graphics window to draw the array in
    button_name_list   = list of strings, 1 for each button name.  This will
                         define how many buttons are in the array
    bot_lft_x          = x coordinate for the bottom left corner of the first
                         button in the array
    bot_lft_y          = y coordinate for the bottom left corner of the first
                         button in the array
    top_rgt_x          = x coordinate for the top right corner of the first
                         button in the array
    top_rgt_y          = y coordinate for the too right corner of the first
                         button in the array
    lne_col            = colour of the buttons outline
    fill_col           = colour of the button fill, note the button text
                         will default to black

button_array_opt_pa
takes a user mouse click point and a specified button array and tests each
button via the clicked_true method.  If a button has been selected that
button is turned red and the function returns the corresponding button
dictionary key of the button array for an if statement to control actions.

definition:
    button_array       = array object being checked to see if selected
    mouseclick         = user mouse click, defined as a point obj
'''


class button_pa:
    def __init__(self, win_obj, bot_lft_x, bot_lft_y, top_rgt_x, top_rgt_y,
                 lne_col, fill_col, butt_number, butt_list):
        self.win_obj = win_obj
        self.bot_lft_x = bot_lft_x
        self.bot_lft_y = bot_lft_y
        self.top_rgt_x = top_rgt_x
        self.top_rgt_y = top_rgt_y
        self.lne_col = lne_col
        self.fill_col = fill_col
        self.p1_x = p1_x
        self.butt_number,
        self.butt_list

    def create_button_pa(win_obj, name, bot_lft_x, bot_lft_y, top_rgt_x,
                         top_rgt_y, lne_col, fill_col):
        rect = Rectangle(Point(bot_lft_x, bot_lft_y), 
                         Point(top_rgt_x, top_rgt_y))
        rect.draw(win_obj)
        rect.setOutline(lne_col)
        rect.setFill(fill_col)
        title = Text(rect.getCenter(), name)
        title.draw(win_obj)
        return rect

    def clicked_true(button_obj_name, mouseclick):
        x_edge_start = button_obj_name.getP1().getX()
        y_edge_start = button_obj_name.getP1().getY()
        y_edge_end = button_obj_name.getP2().getY()
        x_edge_end = button_obj_name.getP2().getX()

        if mouseclick.getX() > x_edge_start and \
         mouseclick.getX() < x_edge_end and \
         mouseclick.getY() > y_edge_start and \
         mouseclick.getY() < y_edge_end:

            button_obj_name.setFill('red')
            selected_button = button_obj_name
            return selected_button
        else:
            return "None"

    def button_array_pa(win_obj, button_name_list, bot_lft_x, bot_lft_y,
                        top_rgt_x, top_rgt_y, lne_col, fill_col):
        if type(button_name_list) != list:
            print("fail_def button_array_pa_array missing list."
                  "Must be a list even or single")
            sys.exit()
        counter = 0
        button_dict = {}
        while counter < len(button_name_list):
            rect = button_pa.create_button_pa(win_obj,
                                              button_name_list[counter],
                                              bot_lft_x, bot_lft_y, top_rgt_x,
                                              top_rgt_y, lne_col, fill_col)
            butt_height = (top_rgt_y-bot_lft_y)
            bot_lft_y = top_rgt_y
            top_rgt_y = (top_rgt_y + butt_height)
            button_dict.update({counter: rect})
            counter = counter + 1
        return button_dict

    def button_array_opt_pa(button_array, mouseclick):
        button_selected = []
        for buttons in range(0, len(button_array)):
            test = button_pa.clicked_true(button_array[buttons], mouseclick)
            button_selected.append(test)
            counter = 0
        for buttons in button_selected:
            if button_selected[counter] != "None":
                button = counter
            else:
                counter = counter + 1
                button = "pass"
